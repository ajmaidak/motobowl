...
Injury status is **not** in the Yahoo paste. Get it from:

1. **Sleeper** `injury_status` per player (`sleeper_players()` in `sources.py`) —
   currently the most reliable source here.
2. **nflverse** `nfl_injuries(season, week)` — richer when populated (practice
...
If neither source has usable status for a player the decision hinges on, **say
so and ask the user** rather than assuming healthy.
